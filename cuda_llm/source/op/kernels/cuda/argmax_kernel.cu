...
__forceinline__ __device__ void warp_reduce_argmax(float& val, size_t& ptr) {
  ...
  for (unsigned int k = (warpSize >> 1); k > 0; k >>= 1) {
    ...
    if (tmp_ptr == SIZE_MAX) continue;
    if (ptr == SIZE_MAX || tmp_val > val) {
      val = tmp_val;
      ptr = tmp_ptr;
...
__global__ void argmax_kernel_fp32(const float* input_ptr, size_t size, size_t* output_idx) {
  ...
  for (size_t i = tid; i < size; i += blockDim.x) {
    if (max_index == SIZE_MAX || input_ptr[i] > max_value) {
      max_index = i;
...
size_t argmax_kernel_cu(const float* input_ptr, size_t size, size_t* device_output_idx,
  ...
  CHECK_NE(device_output_idx, nullptr);
  CHECK_GT(size, 0);
  size_t output_index = 0;
  if (!stream) {
    argmax_kernel_fp32<<<1, 512>>>(input_ptr, size, device_output_idx);
    CHECK(cudaMemcpy(&output_index, device_output_idx, sizeof(size_t), cudaMemcpyDeviceToHost) ==
          cudaSuccess);
  } else {
    cudaStream_t stream_ = static_cast<cudaStream_t>(stream);
    argmax_kernel_fp32<<<1, 512, 0, stream_>>>(input_ptr, size, device_output_idx);
    CHECK(cudaMemcpyAsync(&output_index, device_output_idx, sizeof(size_t),
