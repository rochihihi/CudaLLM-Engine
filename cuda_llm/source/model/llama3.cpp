...
base::Status LLama2Model::forward(const tensor::Tensor& input, const tensor::Tensor& pos_tensor,
  ...
  if (profiling_enabled_) {
    CHECK(device_type_ == base::DeviceType::kDeviceCUDA);
    CHECK_EQ(cudaEventCreate(&start), cudaSuccess);
    CHECK_EQ(cudaEventCreate(&stop), cudaSuccess);
  }

  auto run_stage = [&](auto&& stage, double& elapsed_total_ms) {
    if (!profiling_enabled_) {
      stage();
      return;
    }
    CHECK_EQ(cudaEventRecord(start, cuda_config_->stream), cudaSuccess);
    stage();
    CHECK_EQ(cudaEventRecord(stop, cuda_config_->stream), cudaSuccess);
    CHECK_EQ(cudaEventSynchronize(stop), cudaSuccess);
    float elapsed_ms = 0.0f;
    CHECK_EQ(cudaEventElapsedTime(&elapsed_ms, start, stop), cudaSuccess);
    elapsed_total_ms += elapsed_ms;
  };

  for (int32_t layer_idx = 0; layer_idx < config_->layer_num_; ++layer_idx) {
    run_stage([&] { attention_rms(layer_idx, input); }, profile_.attention_rms_ms);
    run_stage([&] { attention_qkv(layer_idx, pos_tensor); }, profile_.attention_qkv_ms);
    run_stage([&] { attention_mha(layer_idx, pos_tensor); }, profile_.attention_mha_ms);
    if (profiling_enabled_) {
      feed_forward(layer_idx, input, start, stop);
    } else {
      feed_forward(layer_idx, input);
    }
    if (profiling_enabled_) {
      ++profile_.layer_calls;
    }
  }
  run_stage([&] { cls_logits(input); }, profile_.cls_logits_ms);
  if (profiling_enabled_) {
    ++profile_.forward_calls;
...
void LLama2Model::set_profiling_enabled(bool enabled) {
  profiling_enabled_ = enabled;
  profile_ = LLama2Profile{};
}

const LLama2Profile& LLama2Model::profile() const { return profile_; }
...
void LLama2Model::create_param_quant_layers() {
  ...
  // Shared classifier weights are stored once as the FP32 embedding table.
...
void LLama2Model::feed_forward(int32_t layer_idx, const tensor::Tensor& input,
                               cudaEvent_t profile_start, cudaEvent_t profile_stop) const {
  CHECK(llama_layers_ != nullptr);
  auto run_ffn_stage = [&](auto&& stage, double& stage_total_ms) {
    if (!profiling_enabled_) {
      stage();
      return;
    }
    CHECK_NE(profile_start, nullptr);
    CHECK_NE(profile_stop, nullptr);
    CHECK_EQ(cudaEventRecord(profile_start, cuda_config_->stream), cudaSuccess);
    stage();
    CHECK_EQ(cudaEventRecord(profile_stop, cuda_config_->stream), cudaSuccess);
    CHECK_EQ(cudaEventSynchronize(profile_stop), cudaSuccess);
    float elapsed_ms = 0.0f;
    CHECK_EQ(cudaEventElapsedTime(&elapsed_ms, profile_start, profile_stop), cudaSuccess);
    stage_total_ms += elapsed_ms;
    profile_.feed_forward_ms += elapsed_ms;
  };
  ...
  run_ffn_stage([&] { STATUS_CHECK(ffn_rmsnorm->forward(input, ffn_norm_output)); },
                profile_.ffn_rms_ms);
  ...
  run_ffn_stage([&] { STATUS_CHECK(w1_layer->forward(ffn_norm_output, w1_output)); },
                profile_.ffn_w1_w3_ms);
  ...
  run_ffn_stage([&] { STATUS_CHECK(w3_layer->forward(ffn_norm_output, w3_ouput)); },
                profile_.ffn_w1_w3_ms);
  ...
  run_ffn_stage([&] { STATUS_CHECK(w2_layer->forward(w1_output, w2_output)); },
                profile_.ffn_w2_ms);
