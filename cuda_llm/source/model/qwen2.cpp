...
void Qwen2Model::create_param_quant_layers() {
  ...
  for (int32_t i = 0; i < config_->layer_num_; ++i) {
    auto wk =
        std::make_shared<op::MatmulLayer>(device_type_, config_->kv_dim_, dim, true, true);
    ...
    wk->set_bias(0, kv_dim, this->raw_model_data_->weight(pos), cpu_device_type);
    pos += config_->kv_dim_ * sizeof(float);
  }
  ...
  for (int32_t i = 0; i < config_->layer_num_; ++i) {
    ...
    int32_t kv_dim = config_->kv_dim_;
    wv->set_bias(0, kv_dim, this->raw_model_data_->weight(pos), cpu_device_type);
    pos += config_->kv_dim_ * sizeof(float);
  }
